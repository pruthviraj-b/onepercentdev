...
function isNetworkFailure(error: unknown): boolean {
  if (typeof error === 'object' && error !== null && 'retryable' in error && (error as { retryable?: boolean }).retryable) return true;
...
async function fetchApi(input: RequestInfo | URL, init: RequestInit = {}, timeoutMs = 3500): Promise<Response> {
...
export async function fetchTasks(params: FetchTasksParams = {}): Promise<TasksPage> {
  ...
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks?${qs}`, {
      cache: 'no-store',
      headers: getAuthHeaders(),
    });
    if (!res.ok) {
      const error = await res.json().catch(() => ({}));
      const failure = new Error(error.error || `Could not load tasks (${res.status})`) as Error & { retryable?: boolean };
      failure.retryable = res.status >= 500;
      throw failure;
    }
    return res.json();
  } catch (error) {
...
export async function createTask(input: CreateTaskInput): Promise<SmartTask> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks`, {
      method: 'POST',
      headers: getAuthHeaders(),
    ...
    const failure = new Error(`Could not create task (${res.status})`) as Error & { retryable?: boolean };
    failure.retryable = res.status >= 500;
    throw failure;
  } catch (error) {
    if (!isNetworkFailure(error)) throw error;
...
export async function updateTask(id: number, patch: UpdateTaskInput): Promise<SmartTask> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks/${id}`, {
      method: 'PUT',
      headers: getAuthHeaders(),
    ...
    const err = await res.json().catch(() => ({}));
    if (err.error) throw new Error(err.error);
    const failure = new Error(`Could not update task (${res.status})`) as Error & { retryable?: boolean };
    failure.retryable = res.status >= 500;
    throw failure;
  } catch (error) {
    if (!isNetworkFailure(error)) throw error;
...
export async function deleteTask(id: number): Promise<void> {
  try {
    ...
    if (res.ok) return;
    const err = await res.json().catch(() => ({}));
    const failure = new Error(err.error || `Could not delete task (${res.status})`) as Error & { retryable?: boolean };
    failure.retryable = res.status >= 500;
    throw failure;
  } catch (error) {
    if (!isNetworkFailure(error)) throw error;
...
export async function duplicateTask(id: number): Promise<SmartTask> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks/${id}/duplicate`, {
      method: 'POST',
      headers: getAuthHeaders(),
    });
    if (res.ok) return res.json();
    const err = await res.json().catch(() => ({}));
    const failure = new Error(err.error || `Could not duplicate task (${res.status})`) as Error & { retryable?: boolean };
    failure.retryable = res.status >= 500;
    throw failure;
  } catch (error) {
    if (!isNetworkFailure(error)) throw error;
...
export async function bulkUpdateSortOrder(updates: { id: number; sort_order: number }[]): Promise<void> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks/reorder`, {
      method: 'POST',
      headers: getAuthHeaders(),
    ...
    if (res.ok) return;
    const err = await res.json().catch(() => ({}));
    const failure = new Error(err.error || `Could not reorder tasks (${res.status})`) as Error & { retryable?: boolean };
    failure.retryable = res.status >= 500;
    throw failure;
  } catch (error) {
    if (!isNetworkFailure(error)) throw error;
...
export async function fetchAnalytics(): Promise<TaskAnalytics> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks/analytics`, {
      cache: 'no-store',
      headers: getAuthHeaders(),
    });
...
export async function fetchLinkPreview(url: string): Promise<{
  ...
  try {
    const res = await fetchApi(`${getApiBase()}/api/link-preview`, {
      method: 'POST',
      headers: getAuthHeaders(),
...
export async function fetchNotificationPreferences(): Promise<NotificationPreferences> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/notification-preferences`, {
      cache: 'no-store',
      headers: getAuthHeaders(),
    });
...
export async function fetchReminderStatus(): Promise<ReminderStatus> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/reminders/status`, {
      cache: 'no-store',
      headers: getAuthHeaders(),
    });
    if (res.ok) return res.json();
...
export async function fetchReminderHistory(): Promise<ReminderHistoryItem[]> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/reminders/history`, {
      cache: 'no-store',
      headers: getAuthHeaders(),
    });
    if (res.ok) return res.json();
...
export async function sendTestReminderEmail(email_address: string): Promise<{
  ...
  const res = await fetchApi(`${getApiBase()}/api/reminders/test-email`, {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function notifyLmsComplete(params: {
  ...
  try {
    await fetchApi(`${getApiBase()}/api/smart-tasks/lms-complete`, {
      method: 'POST',
      headers: getAuthHeaders(),
...
export async function fetchUserTags(): Promise<string[]> {
  try {
    const res = await fetchApi(`${getApiBase()}/api/smart-tasks/tags`, {
      cache: 'no-store',
      headers: getAuthHeaders(),
    });
