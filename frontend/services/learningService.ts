...
let tasksRequest: Promise<Task[]> | null = null;
let streakRequest: Promise<StreakInfo> | null = null;
...
export async function fetchTasks(): Promise<Task[]> {
  ...
  try { return await tasksRequest; } finally { tasksRequest = null; }
}
...
export async function fetchStreak(): Promise<StreakInfo> {
  if (streakRequest) return streakRequest;
  streakRequest = (async () => {
    try {
      const controller = new AbortController();
      const timeout = window.setTimeout(() => controller.abort(), 3500);
  ...
  try { return await streakRequest; } finally { streakRequest = null; }
}
