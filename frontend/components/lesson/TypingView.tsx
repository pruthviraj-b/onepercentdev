...
import { submitTypingScore, pingStreak } from '@/services/learningService';
...
const C = {
  ...
  textFaint: '#9CA3AF',
  neonCyan: '#F98012',
  cyanGlow: 'rgba(255, 104, 66, 0.13)',
  cyanGlow2: 'rgba(255, 104, 66, 0.07)',
  neonGreen: '#22C55E',
  greenGlow: 'rgba(45, 150, 117, 0.12)',
  neonPurple: '#776C86',
  purpleGlow: 'rgba(119, 108, 134, 0.12)',
  neonYellow: '#F59E0B',
  neonRed: '#EF4444',
  onAccent: '#FFFFFF',
};
    ...
    <div className="typing-page" style={{ background: `radial-gradient(900px at 18% -10%, ${C.cyanGlow}, transparent), ${C.bg}`, minHeight: '100vh', color: C.text, fontFamily: 'var(--font-ui)' }}>
      ...
      <div className="typing-layout" style={{ display: 'grid', gridTemplateColumns: '280px 1fr', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <main className="typing-main" style={{ padding: 24, maxWidth: 1020 }}>
          ...
          {!activeDailySet && gameState === 'lesson' && (
            <div className="typing-lesson-widget" style={{ background: `linear-gradient(180deg, ${C.surfaceCardHi}, ${C.surfaceCard})`, border: `1px solid ${C.border}`, borderRadius: 16, padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ marginTop: 16, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14 }}>
                <code style={{ fontFamily: 'Google Sans Flex, sans-serif', color: C.neonCyan }}>{currentLesson.syntax}</code>
              </div>
          ...
          <div className="typing-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 18 }}>
            {[
              ...
              <div key={s.k} className="typing-stat-widget" style={{ background: C.surfaceCard, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14 }}>
                <div style={{ fontSize: '0.65rem', color: C.textFaint, fontWeight: 800 }}>{s.k}</div>
                <div style={{ fontFamily: 'Google Sans Flex, sans-serif', fontSize: '1.4rem', fontWeight: 900, color: s.c as string }}>{s.v as any}</div>
              </div>
            ))}
          </div>

          <div className="typing-target-widget" style={{ background: C.surfaceCard, border: `1px solid ${isFocused? C.neonCyan : C.border}`, borderRadius: 14, padding: 20, fontFamily: 'Google Sans Flex, sans-serif', fontSize: '0.92rem', lineHeight: 1.85, position: 'relative' }}>
          ...
          <textarea className="typing-input-widget" aria-label="Typing practice input" ref={textareaRef} value={userInput} onChange={e => setUserInput(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} disabled={gameState === 'finished'} placeholder="Start typing..." autoComplete="off" spellCheck={false} style={{ width: '100%', minHeight: 140, padding: 16, marginTop: 16, border: `1px solid ${isFocused? C.neonCyan : C.border}`, background: C.surfaceCard, color: C.text, fontFamily: 'Google Sans Flex, sans-serif', outline: 'none', borderRadius: 12, resize: 'vertical' }} />

          <div className="typing-progress-widget" style={{ height: 8, background: C.surface, borderRadius: 6, marginTop: 14, overflow: 'hidden', border: `1px solid ${C.border}` }}><div style={{ height: '100%', width: `${Math.min(100, (correctChars / Math.max(1, target.length)) * 100)}%`, background: C.neonCyan }} /></div>
        </main>
      </div>
    </div>
  );
