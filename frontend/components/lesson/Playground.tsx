  ...
  pyodideReady = (async () => {
    if (!(window as any).loadPyodide) {
      await new Promise<void>((res, rej) => {
        ...
        s.onload = () => res();
        s.onerror = () => rej(new Error('Could not load the Python runtime. Check your network connection and try again.'));
  ...
  sqlPromise = (async () => {
    if (!(window as any).initSqlJs) {
      await new Promise<void>((res, rej) => {
        ...
        s.onload = () => res();
        s.onerror = () => rej(new Error('Could not load the SQL runtime. Check your network connection and try again.'));
        document.head.appendChild(s);
      });
    }
...
export function Playground({ prefilledCode, courseId }: Props) {
  ...
  return (
    <div className={`playground hacker-theme ${scanlines ? 'crt-scanlines' : ''}`}>
      ...
      {isCloud ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#1F2937', border: '2px solid #1F2937', borderRadius: '0', minHeight: 0 }}>
          <div className="playground-header hacker-header" style={{ background: '#1F2937', borderBottom: '1px solid #333333', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', textAlign: 'center', overflowY: 'auto' }}>
            ...
            <a 
              ...
              style={{ padding: '8px 24px', height: 'auto', fontSize: '0.85rem', background: '#ffffff', color: '#1F2937', textDecoration: 'none', fontWeight: 'bold', flexShrink: 0 }}
            >
              Launch Terminal ↗
            </a>
          </div>
        ...
        <>
          ...
          <div className="output-panel" style={{ borderTopColor: '#333333' }}>
            ...
            <div
              ...
              ref={outputRef}
              style={{ background: '#1F2937', padding: '10px 14px', fontFamily: 'Google Sans Flex, sans-serif' }}
            >
              {output.length === 0 ? (
