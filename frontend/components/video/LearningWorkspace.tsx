...
import { LearningPlayer } from '@/components/video/LearningPlayer';
import { TimestampNotes } from '@/components/reader/TimestampNotes';
import { TimestampBookmarks } from '@/components/reader/TimestampBookmarks';
import { PomodoroTimer } from '@/components/video/PomodoroTimer';
import { formatSeconds } from '@/services/readerService';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, background: 'transparent', fontFamily: 'var(--font-ui)' }}>
      ...
      <div style={{
        ...
        <span style={{ color: '#6B7280', fontFamily: 'var(--font-mono)', fontSize: '0.68rem', flexShrink: 0 }}>
          ...
          {panelButtons.map(btn => (
            <button key={btn.id}
              ...
              style={{
                ...
                borderColor: sidePanel === btn.id ? '#1F2937' : 'rgba(0,0,0,0.2)',
                background: 'transparent',
                color: sidePanel === btn.id ? '#1F2937' : '#6B7280',
...
function LearningPlayerWithSeek(props: {
  ...
  useEffect(() => {
    window.addEventListener('lp-seek', handleSeekEvent);
    return () => window.removeEventListener('lp-seek', handleSeekEvent);
  }, [handleSeekEvent]);
