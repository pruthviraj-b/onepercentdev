    ...
    <div style={{ fontFamily: 'var(--font-ui)' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          ...
          style={{ width: '100%', border: '2px solid #1F2937', padding: '8px 12px', fontFamily: 'var(--font-ui)', fontSize: '0.88rem', outline: 'none', boxSizing: 'border-box' }}
        />
      </div>
      ...
      {loading && page === 1 ? (
        ...
        <div style={{ padding: '40px 20px', textAlign: 'center', color: '#9CA3AF', border: '2px dashed #D1D5DB' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {history.map(entry => (
            <div key={`${entry.course_id}-${entry.part_id}`}
              style={{ border: '2px solid #1F2937', background: '#fff', display: 'flex', gap: 12, padding: 12, position: 'relative' }}>
              {/* Thumbnail */}
              <div style={{ flexShrink: 0, width: 120, height: 68, position: 'relative', background: '#1F2937', overflow: 'hidden' }}>
                {entry.thumbnail_url && (
                ...
                {entry.duration_seconds && entry.duration_seconds > 0 && (
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, background: 'rgba(0,0,0,0.4)' }}>
                    <div style={{ height: '100%', width: `${Math.min(100, entry.percent_watched)}%`, background: entry.is_completed ? '#22C55E' : '#F59E0B' }} />
                  </div>
                )}
                {entry.is_completed && (
                  <div style={{ position: 'absolute', top: 4, right: 4, background: '#22C55E', color: '#fff', fontSize: '0.6rem', fontWeight: 900, padding: '2px 5px' }}>✓</div>
                )}
                {/* Resume time */}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '0.65rem', color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>
                  {entry.course_title}
                </div>
                <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1F2937', marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {entry.lesson_title || `Part ${entry.part_id}`}
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.7rem', color: '#6B7280' }}>{relativeTime(entry.last_watched_at)}</span>
                  {entry.duration_seconds && (
                    <span style={{ fontSize: '0.7rem', color: '#6B7280' }}>{formatDuration(entry.duration_seconds)}</span>
                  )}
                  {entry.percent_watched > 0 && (
                    <span style={{ fontSize: '0.7rem', fontWeight: 700, color: entry.is_completed ? '#22C55E' : '#F59E0B' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                <button onClick={() => onResume(entry.course_id, entry.part_id)}
                  style={{ padding: '5px 12px', background: '#1F2937', color: '#F59E0B', border: '2px solid #1F2937', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.7rem', cursor: 'pointer', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
          ...
          {history.length < total && (
            <button onClick={() => { const np = page + 1; setPage(np); load(np); }}
              disabled={loading}
              style={{ padding: '10px', border: '2px solid #1F2937', background: '#FFFFFF', fontFamily: 'var(--font-ui)', fontWeight: 800, fontSize: '0.82rem', cursor: 'pointer', textTransform: 'uppercase' }}>
