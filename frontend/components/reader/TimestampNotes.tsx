    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'transparent', fontFamily: 'var(--font-ui)' }}>
      ...
      {adding && (
        <div style={{ padding: '10px 14px', background: 'transparent', borderBottom: '1px solid rgba(0,0,0,0.1)', flexShrink: 0 }}>
          <div style={{ fontSize: '0.7rem', color: '#6B7280', marginBottom: 4, fontFamily: 'var(--font-mono)' }}>
          ...
          <textarea
            ...
            autoFocus
            rows={3}
            style={{ width: '100%', border: '2px solid #1F2937', padding: '8px', fontFamily: 'var(--font-content)', fontSize: '0.85rem', resize: 'vertical', outline: 'none', boxSizing: 'border-box' }}
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
            <button onClick={handleAdd} disabled={!newContent.trim() || saving}
              style={{ padding: '5px 14px', background: '#1F2937', color: '#F59E0B', border: '2px solid #1F2937', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.72rem', cursor: 'pointer', textTransform: 'uppercase', opacity: !newContent.trim() ? 0.4 : 1 }}>
      ...
      {/* Search */}
      {notes.length > 3 && (
        <div style={{ padding: '8px 14px', borderBottom: '1px solid #E5E7EB', flexShrink: 0 }}>
          <input
            type="text" value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search notes..."
            style={{ width: '100%', border: '2px solid #E5E7EB', padding: '5px 10px', fontFamily: 'var(--font-ui)', fontSize: '0.8rem', outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
        {loading && (
          <div style={{ padding: '20px', textAlign: 'center', color: '#6B7280', fontSize: '0.82rem' }}>Loading notes…</div>
        )}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                  <button onClick={() => handleEdit(note.id)} disabled={saving}
                    style={{ padding: '4px 12px', background: '#1F2937', color: '#F59E0B', border: '2px solid #1F2937', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.7rem', cursor: 'pointer' }}>
                    Save
                  </button>
                  <button onClick={() => { setEditingId(null); setEditContent(''); }}
                    style={{ padding: '4px 10px', background: 'transparent', color: '#6B7280', border: '2px solid #D1D5DB', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.7rem', cursor: 'pointer' }}>
                    Cancel
                  </button>
                </div>
              </div>
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                  <button
                    ...
                    style={{ padding: '2px 8px', background: '#1F2937', color: '#F59E0B', border: '2px solid #1F2937', fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '0.7rem', cursor: 'pointer', flexShrink: 0 }}>
                  ...
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
                    <button onClick={() => { setEditingId(note.id); setEditContent(note.content); }}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', color: '#6B7280', padding: '2px 4px' }} title="Edit">
                      ✏️
                    </button>
                ...
                <p style={{ margin: 0, fontSize: '0.85rem', lineHeight: 1.5, color: '#1F2937', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                  {note.content}
                </p>
              </>
            )}
