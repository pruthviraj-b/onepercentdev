...
const DEFAULT_CATEGORIES: BookmarkCategory[] = [
  ...
  { name: 'Exam', color: '#F98012' },
  { name: 'Revision', color: '#3B82F6' },
  { name: 'Assignment', color: '#F59E0B' },
  { name: 'Important', color: '#F59E0B' },
  { name: 'Formula', color: '#22C55E' },
...
export function TimestampBookmarks({ courseId, partId, videoId, currentTime, onSeek }: TimestampBookmarksProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'transparent', fontFamily: 'var(--font-ui)' }}>
      {/* Header */}
      <div style={{ background: 'transparent', color: '#1F2937', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, borderBottom: '1px solid rgba(0,0,0,0.1)' }}>
        <div style={{ fontWeight: 900, fontSize: '0.82rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#1F2937' }}>
          🔖 Bookmarks {bookmarks.length > 0 && <span style={{ opacity: 0.5, fontWeight: 400 }}>({bookmarks.length})</span>}
        </div>
        <button onClick={() => setShowAdd(a => !a)}
          style={{ background: 'transparent', color: '#1F2937', border: '2px solid #1F2937', padding: '4px 10px', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.7rem', cursor: 'pointer', textTransform: 'uppercase' }}>
      ...
      {showAdd && (
        <div style={{ padding: '10px 14px', background: 'transparent', borderBottom: '1px solid rgba(0,0,0,0.1)', flexShrink: 0 }}>
          <div style={{ fontSize: '0.7rem', color: '#6B7280', marginBottom: 6 }}>Adding bookmark at ⏱ {formatSeconds(currentTime)}</div>
          <input type="text" value={newLabel} onChange={e => setNewLabel(e.target.value)}
            placeholder="Label (optional)"
            style={{ width: '100%', border: '2px solid #1F2937', padding: '6px 10px', fontFamily: 'var(--font-ui)', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box', marginBottom: 8 }}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
            {DEFAULT_CATEGORIES.map(cat => (
              <button key={cat.name} onClick={() => setNewCategory(cat.name)}
                style={{ padding: '3px 8px', border: '2px solid', borderColor: newCategory === cat.name ? cat.color : '#E5E7EB', background: newCategory === cat.name ? cat.color : '#fff', color: newCategory === cat.name ? '#fff' : '#6B7280', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.65rem', cursor: 'pointer' }}>
                {cat.name}
              </button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleAdd}
              style={{ padding: '5px 14px', background: '#1F2937', color: '#F59E0B', border: '2px solid #1F2937', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.72rem', cursor: 'pointer' }}>
              Save
            </button>
            <button onClick={() => { setShowAdd(false); setNewLabel(''); }}
              style={{ padding: '5px 10px', background: 'transparent', color: '#6B7280', border: '2px solid #D1D5DB', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.72rem', cursor: 'pointer' }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {(usedCategories.length > 1 || bookmarks.length > 4) && (
        <div style={{ padding: '8px 14px', borderBottom: '1px solid #E5E7EB', display: 'flex', gap: 6, flexShrink: 0, flexWrap: 'wrap' }}>
          {usedCategories.length > 1 && (
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              <button onClick={() => setFilterCategory('')}
                style={{ padding: '2px 8px', border: '2px solid', borderColor: !filterCategory ? '#1F2937' : '#D1D5DB', background: !filterCategory ? '#1F2937' : '#fff', color: !filterCategory ? '#fff' : '#6B7280', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.65rem', cursor: 'pointer' }}>
                All
              </button>
              {usedCategories.map(cat => {
                const c = categories.find(c => c.name === cat) || { color: '#6B7280' };
                return (
                  <button key={cat} onClick={() => setFilterCategory(filterCategory === cat ? '' : cat)}
                    style={{ padding: '2px 8px', border: '2px solid', borderColor: filterCategory === cat ? c.color : '#E5E7EB', background: filterCategory === cat ? c.color : '#fff', color: filterCategory === cat ? '#fff' : '#6B7280', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.65rem', cursor: 'pointer' }}>
                    {cat}
                  </button>
                );
              })}
          ...
          {bookmarks.length > 4 && (
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search…"
              style={{ flex: 1, minWidth: 80, border: '2px solid #E5E7EB', padding: '3px 8px', fontFamily: 'var(--font-ui)', fontSize: '0.78rem', outline: 'none' }}
            />
          )}
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 0' }}>
        {loading && <div style={{ padding: '20px', textAlign: 'center', color: '#6B7280', fontSize: '0.82rem' }}>Loading…</div>}
        ...
        {filtered.map(bk => {
          const cat = categories.find(c => c.name === bk.category) || { color: '#F59E0B' };
          return (
                ...
                {editingId === bk.id ? (
                  <>
                    <input type="text" value={editLabel} onChange={e => setEditLabel(e.target.value)}
                      style={{ width: '100%', border: '2px solid #1F2937', padding: '4px 8px', fontFamily: 'var(--font-ui)', fontSize: '0.8rem', outline: 'none', boxSizing: 'border-box', marginBottom: 6 }}
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginBottom: 6 }}>
                      {DEFAULT_CATEGORIES.map(c => (
                        <button key={c.name} onClick={() => setEditCategory(c.name)}
                          style={{ padding: '2px 6px', border: '2px solid', borderColor: editCategory === c.name ? c.color : '#E5E7EB', background: editCategory === c.name ? c.color : '#fff', color: editCategory === c.name ? '#fff' : '#6B7280', fontSize: '0.6rem', fontFamily: 'var(--font-ui)', cursor: 'pointer' }}>
                          {c.name}
                        </button>
                      ))}
                    </div>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button onClick={() => handleSaveEdit(bk.id)} style={{ padding: '3px 10px', background: '#1F2937', color: '#F59E0B', border: '2px solid #1F2937', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.65rem', cursor: 'pointer' }}>Save</button>
                      <button onClick={() => setEditingId(null)} style={{ padding: '3px 8px', background: 'transparent', color: '#6B7280', border: '2px solid #D1D5DB', fontSize: '0.65rem', cursor: 'pointer' }}>Cancel</button>
                    </div>
                  </>
                ) : (
                  <>
                    <button onClick={() => onSeek(bk.timestamp_sec)}
                      ...
                      <div style={{ fontSize: '0.82rem', fontWeight: 600, color: '#1F2937', marginTop: 2, wordBreak: 'break-word' }}>
