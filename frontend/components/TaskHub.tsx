...
const TIME_FILTERS: { key: TimeFilter; label: string }[] = [
  ...
  { key: 'tomorrow',   label: 'ðŸŒ… Tomorrow' },
  { key: 'this_week',  label: 'ðŸ“† This Week' },
  { key: 'next_week',  label: 'ðŸ—“ï¸ Next Week' },
  { key: 'this_month', label: 'ðŸ“† This Month' },
  { key: 'all',        label: 'ðŸ—‚ï¸ All Tasks' },
];
...
const PRIORITY_LABELS: Record<TaskPriority, string> = {
  low: 'ðŸŸ¢ Low', medium: 'ðŸŸ¡ Medium', high: 'ðŸŸ  High', critical: 'ðŸ”´ Critical',
};
const STATUS_LABELS: Record<TaskStatus, string> = {
  not_started: 'â¬œ Not Started', in_progress: 'ðŸ”µ In Progress',
  completed: 'âœ… Completed', skipped: 'â­ï¸ Skipped', archived: 'ðŸ“¦ Archived',
};
...
const INTERNAL_TARGET_LABELS: Record<string, string> = {
  course: 'ðŸ“˜ Course', module: 'ðŸ“‚ Module', lesson: 'ðŸ“„ Lesson',
  quiz: 'ðŸ§ª Quiz', assignment: 'ðŸ“ Assignment', project: 'ðŸ—ï¸ Project',
  practice_lab: 'ðŸ”¬ Practice Lab', certificate: 'ðŸ† Certificate', dashboard: 'ðŸ  Dashboard',
};
...
// â”€â”€ Link Preview Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    >
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 12px', cursor:'pointer' }}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', flexWrap:'wrap', gap:6, marginBottom:3 }}>
            ...
            {task.is_pinned && <span title="Pinned" style={{ fontSize:'0.7rem' }}>ðŸ“Œ</span>}
            ...
            {task.link_type === 'internal' && (
              <span title="Internal LMS link" style={{ fontSize:'0.75rem' }}>ðŸ”—</span>
            )}
            {task.recurrence_rule && task.recurrence_rule !== 'none' && (
              <span title={`Recurring: ${task.recurrence_rule}`} style={{ fontSize:'0.7rem' }}>ðŸ”„</span>
            )}
          </div>
        ...
        <div style={{ display:'flex', gap:4, flexShrink:0 }}>
          <button onClick={e => { e.stopPropagation(); setMenuOpen(m => !m); }}
            ...
            aria-label="Task menu" title="More options">â‹®</button>
        </div>
      </div>
      ...
      {menuOpen && (
        <div style={{ borderTop:'1px solid #e0e0e0', padding:'6px 12px', display:'flex', flexWrap:'wrap', gap:6 }}
          onClick={e => e.stopPropagation()}>
          {[
            { label:'âœï¸ Edit',       action: () => { onEdit(task); setMenuOpen(false); } },
            { label:'ðŸ“‹ Duplicate',  action: () => { onDuplicate(task); setMenuOpen(false); } },
            { label: task.is_pinned ? 'ðŸ“Œ Unpin' : 'ðŸ“Œ Pin', action: () => { onPin(task); setMenuOpen(false); } },
            { label: task.is_archived ? 'â™»ï¸ Restore' : 'ðŸ“¦ Archive', action: () => { onArchive(task); setMenuOpen(false); } },
            { label:'ðŸ—‘ï¸ Delete',    action: () => { onDelete(task); setMenuOpen(false); } },
          ].map(item => (
        ...
        <div style={{ borderTop:'2px solid #e0e0e0', padding:'12px 14px', background:'#fafaf8' }}
          ...
          {task.link_type === 'internal' && task.internal_link_target && (
            <div style={{ marginTop:8, fontSize:'0.8rem' }}>
              ðŸ”— <strong>Internal:</strong> {INTERNAL_TARGET_LABELS[task.internal_link_target] || task.internal_link_target}
              {task.internal_link_label && <span> â€” {task.internal_link_label}</span>}
          ...
          <div style={{ display:'flex', gap:12, marginTop:8, fontSize:'0.72rem', color:'#888', fontFamily:'var(--font-mono)' }}>
            {task.due_time && <span>ðŸ• {task.due_time}</span>}
            {task.recurrence_rule && task.recurrence_rule !== 'none' && <span>ðŸ”„ {task.recurrence_rule}</span>}
...
// â”€â”€ Task Form (Create / Edit) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      <form onSubmit={handleSubmit} style={{ background:'#fff', border:'3px solid #000',
        ...
        <div style={{ background:'#f1be3e', borderBottom:'3px solid #000', padding:'12px 18px',
          ...
          <span style={{ fontWeight:900, fontSize:'1rem', textTransform:'uppercase', letterSpacing:'0.05em' }}>
            {isEdit ? 'âœï¸ Edit Task' : '+ New Task'}
          </span>
        ...
        <div style={{ display:'flex', gap:4, padding:'10px 16px 0', borderBottom:'2px solid #e0e0e0', flexShrink:0 }}>
          {(['basic','schedule','link','details'] as const).map(s => (
            <button key={s} type="button" style={sectionBtnStyle(activeSection===s)}
              ...
              {{ basic:'ðŸ“‹ Basic', schedule:'ðŸ“… Schedule', link:'ðŸ”— Link', details:'ðŸ“ Details' }[s]}
            </button>
          ))}
        </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                ...
                <div>
                  ...
                  <select style={inputStyle} value={form.course_id || ''} onChange={e => set('course_id', e.target.value || '')}>
                    ...
                    {courses.filter(c => !['data-analyst','data-analyst-en'].includes(c.id)).map(c => <option key={c.id} value={c.id}>{c.mascot || 'ðŸ“˜'} {c.title}</option>)}
                  </select>
                </div>
              </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  {([null,'internal','external'] as (LinkType|null)[]).map(lt => (
                    <button key={String(lt)} type="button"
                      ...
                      {lt === null ? 'â€” None' : lt === 'internal' ? 'ðŸ« Internal' : 'ðŸŒ External'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {form.link_type === 'external' && (
                <>
                  ...
                  {previewLoading && <div style={{ fontSize:'0.8rem', color:'#777', fontFamily:'var(--font-mono)' }}>Fetching previewâ€¦</div>}
                  {preview && (
                  ...
                  {form.tags.map(tag => (
                    <span key={tag} style={{ fontSize:'0.72rem', background:'#f1be3e', padding:'2px 8px', border:'1px solid #000', display:'flex', alignItems:'center', gap:4 }}>
                      ...
                      <button type="button" onClick={() => set('tags', form.tags.filter(t => t !== tag))}
                        style={{ background:'none', border:'none', cursor:'pointer', padding:0, fontSize:'0.7rem', lineHeight:1 }}>âœ•</button>
                    </span>
                  ))}
                </div>
...
// â”€â”€ Analytics Panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

function AnalyticsPanel({ analytics }: { analytics: TaskAnalytics | null }) {
  if (!analytics) return null;
  const stats = [
    { label:'Completed',  value: analytics.completed_tasks, sub:`${analytics.completion_pct}% done`, icon:'âœ…' },
    { label:'Daily Streak', value: analytics.daily_streak, sub:`${analytics.weekly_streak} week streak`, icon:'ðŸ”¥' },
    { label:'Study Hours', value: `${analytics.study_hours}h`, sub:'Estimated', icon:'â±ï¸' },
    { label:'Completion', value: `${analytics.completion_pct}%`, sub:`${analytics.total_tasks} total tasks`, icon:'ðŸ“Š' },
  ];
  return (
    <div style={{ background:'#fff', border:'3px solid #000', boxShadow:'4px 4px 0 #000', marginBottom:20 }}>
      ...
      {(analytics.most_studied_course || analytics.most_used_resource_type) && (
        <div style={{ borderTop:'1px solid #e0e0e0', padding:'10px 16px', display:'flex', gap:16, flexWrap:'wrap', fontSize:'0.78rem' }}>
          {analytics.most_studied_course && <span>ðŸ† Top course: <strong>{analytics.most_studied_course}</strong></span>}
...
// â”€â”€ Main TaskHub Component â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function ReminderSettingsPanel({
  ...
  status: ReminderStatus | null;   // kept in signature for call-site compat, not used
  ...
  // Auto-save 700ms after any change â€” no manual Save button needed
  ...
  const labelSt: React.CSSProperties = {
    ...
    letterSpacing: '0.06em', color: '#555', marginBottom: 6, display: 'block',
  };
  const inputSt: React.CSSProperties = {
    width: '100%', border: '2px solid #000', padding: '9px 11px',
  ...
  const OffsetChip = ({ minutes, label }: { minutes: number; label: string }) => {
    ...
    return (
      <button type="button" onClick={toggle} style={{
        border: '2px solid #000', padding: '7px 13px', fontWeight: 700,
        fontSize: '0.75rem', cursor: 'pointer', fontFamily: 'var(--font-ui)',
        background: active ? '#000' : '#fff', color: active ? '#f1be3e' : '#555',
        transition: 'background 80ms',
      }}>{label}</button>
    );
  };

  // placeholder â€” browserReady used only to determine button label below
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ border: '2px solid #000', background: '#fff' }}>
        <div style={{ background: '#f1be3e', borderBottom: '2px solid #000', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontWeight: 900, fontSize: '0.82rem', textTransform: 'uppercase' }}>ðŸ”” In-App Alerts</span>
          <span style={{ fontSize: '0.68rem', fontWeight: 800, padding: '2px 8px', background: browserReady ? '#00aa44' : '#888', color: '#fff' }}>
            {browserReady ? 'ON' : 'OFF'}
          </span>
        </div>
        <div style={{ padding: '14px' }}>
          <p style={{ margin: '0 0 12px', fontSize: '0.8rem', color: '#555', lineHeight: 1.5 }}>
            Get notified right inside the app when a task is due. No email needed.
          </p>
          <button type="button" onClick={onTestBrowser} style={{
            width: '100%', border: '2px solid #000', padding: '9px',
            fontWeight: 800, fontSize: '0.78rem', cursor: 'pointer',
            fontFamily: 'var(--font-ui)', textTransform: 'uppercase',
            background: browserReady ? '#000' : '#f1be3e',
            color: browserReady ? '#f1be3e' : '#000',
          }}>
            {browserReady ? 'âœ“ Alerts Active â€” Send Test' : 'Turn On In-App Alerts'}
          </button>
        </div>
      </div>

      {/* â”€â”€ Email reminders â”€â”€ */}
      <div style={{ border: '2px solid #000', background: '#fff' }}>
        <div style={{ background: '#1a1a1a', borderBottom: '2px solid #000', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontWeight: 900, fontSize: '0.82rem', textTransform: 'uppercase', color: '#f1be3e' }}>âœ‰ï¸ Email Reminders</span>
          <span style={{ fontSize: '0.68rem', fontWeight: 800, padding: '2px 8px', background: prefs.email_enabled && prefs.email_address ? '#00aa44' : '#888', color: '#fff' }}>
        ...
        <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            ...
            <input
              ...
              style={inputSt}
            />
            <p style={{ margin: '5px 0 0', fontSize: '0.72rem', color: '#888' }}>
              Type your email â€” reminders will go here automatically when tasks are due.
            </p>
          </div>
          {prefs.email_address && (
            <button type="button" onClick={onTestEmail} disabled={testingEmail} style={{
              border: '2px solid #000', padding: '9px', fontWeight: 800,
              fontSize: '0.78rem', cursor: testingEmail ? 'wait' : 'pointer',
              fontFamily: 'var(--font-ui)', textTransform: 'uppercase',
              background: '#000', color: '#f1be3e', width: '100%',
            }}>
              {testingEmail ? 'Sendingâ€¦' : `Send Test â†’ ${prefs.email_address}`}
            </button>
          )}
        </div>
      </div>

      {/* â”€â”€ When to remind â”€â”€ */}
      <div style={{ border: '2px solid #000', background: '#fff' }}>
        <div style={{ background: '#f4f1ea', borderBottom: '2px solid #000', padding: '10px 14px' }}>
          <span style={{ fontWeight: 900, fontSize: '0.82rem', textTransform: 'uppercase' }}>â° Remind Me</span>
        </div>
        <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <p style={{ margin: 0, fontSize: '0.78rem', color: '#555' }}>Before the task is due:</p>
      ...
      <div style={{ border: '2px solid #000', background: '#fff' }}>
        <div style={{ background: '#f4f1ea', borderBottom: '2px solid #000', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontWeight: 900, fontSize: '0.82rem', textTransform: 'uppercase' }}>ðŸ“‹ Morning Summary Email</span>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', userSelect: 'none' }}>
            ...
            <span style={{ fontSize: '0.72rem', fontWeight: 800 }}>{prefs.daily_digest_enabled ? 'ON' : 'OFF'}</span>
          </label>
        </div>
        {prefs.daily_digest_enabled && (
          <div style={{ padding: '14px', display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <p style={{ margin: 0, fontSize: '0.75rem', color: '#777', lineHeight: 1.5 }}>
      ...
      {/* Auto-save indicator */}
      {saving && (
        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: '#aaa', fontWeight: 700 }}>Savingâ€¦</div>
      )}

      {/* Recent reminders sent (simplified) */}
      {history.length > 0 && (
        <div style={{ border: '1px solid #e8e4dc', background: '#fff' }}>
          <div style={{ padding: '9px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #eee' }}>
            <span style={{ fontWeight: 800, fontSize: '0.72rem', textTransform: 'uppercase', color: '#888' }}>Recently Sent</span>
            <button type="button" onClick={onRefreshHistory}
              style={{ border: '1px solid #ccc', background: '#f4f1ea', padding: '3px 8px', fontSize: '0.68rem', fontWeight: 700, cursor: 'pointer' }}>
              Refresh
            </button>
          </div>
            ...
            {history.slice(0, 5).map(item => (
              <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.75rem' }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0,
                  background: item.status === 'sent' ? '#00aa44' : item.status === 'failed' ? '#cc0000' : '#aaa' }} />
                <span style={{ flex: 1, color: '#444' }}>
                  {item.channel === 'email' ? 'âœ‰ï¸' : 'ðŸ””'}{' '}
                  {item.task_id ? `Task #${item.task_id}` : 'Daily digest'}
                </span>
                <span style={{ color: '#bbb', whiteSpace: 'nowrap', fontFamily: 'var(--font-mono)', fontSize: '0.68rem' }}>
...
export function TaskHub({ onBack, onNavigateInternal, courses = [] }: TaskHubProps) {
  ...
  const [showAnalytics, setShowAnalytics] = useState(true);   // always visible in right panel
  const [showReminderSettings, setShowReminderSettings] = useState(false); // kept for compat
  ...
  // Load analytics on mount (always visible in right panel)
  useEffect(() => {
    if (!analytics) fetchAnalytics().then(setAnalytics);
  ...
  const handleToggleStatus = useCallback(async (task: SmartTask) => {
    ...
    setTasks(prev => prev.map(t => t.id === updated.id ? updated : t));
    if (newStatus === 'completed') showToast('ðŸŽ‰ Task completed!');
    fetchAnalytics().then(setAnalytics);
  }, [showToast]);
  ...
  const handlePin = useCallback(async (task: SmartTask) => {
    ...
    setTasks(prev => prev.map(t => t.id === updated.id ? updated : t));
    showToast(updated.is_pinned ? 'ðŸ“Œ Pinned!' : 'ðŸ“Œ Unpinned');
  }, [showToast]);
  ...
  const handleArchive = useCallback(async (task: SmartTask) => {
    ...
    showToast(updated.is_archived ? 'ðŸ“¦ Archived' : 'â™»ï¸ Restored');
  ...
  return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:'#f4f1ea', fontFamily:'var(--font-ui)', overflow:'hidden' }}>

      {/* â”€â”€ Toast â”€â”€ */}
      {toast && (
        <div style={{ position:'fixed', top:16, left:'50%', transform:'translateX(-50%)', background:'#000', color:'#f1be3e',
          padding:'9px 20px', border:'2px solid #f1be3e', fontWeight:700, fontSize:'0.85rem', zIndex:1000,
          boxShadow:'4px 4px 0 #f1be3e', whiteSpace:'nowrap' }}
          role="status" aria-live="polite">{toast}</div>
      )}

      {/* â”€â”€ Web alerts (top-right) â”€â”€ */}
      {webAlerts.length > 0 && (
        <div style={{ position:'fixed', top:72, right:16, width:'min(320px,calc(100vw - 32px))',
          ...
          {webAlerts.map(alert => (
            <div key={alert.id} style={{ background:'#fff', border:'3px solid #000', boxShadow:'4px 4px 0 #f1be3e', padding:'10px 12px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', gap:8, alignItems:'flex-start' }}>
                <div>
                  <div style={{ fontWeight:900, fontSize:'0.8rem', textTransform:'uppercase', marginBottom:3 }}>{alert.title}</div>
                  <div style={{ fontSize:'0.76rem', lineHeight:1.4, fontWeight:600 }}>{alert.body}</div>
                </div>
                <button type="button" onClick={() => setWebAlerts(prev => prev.filter(a => a.id !== alert.id))}
                  style={{ border:'2px solid #000', background:'#f1be3e', cursor:'pointer', fontWeight:900, padding:'0 5px', lineHeight:'18px' }}>Ã—</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* â”€â”€ Top nav â”€â”€ */}
      <nav style={{ flexShrink:0, background:'#f1be3e', borderBottom:'3px solid #000',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <button onClick={onBack} style={{ background:'#000', color:'#f1be3e', border:'2px solid #000',
            padding:'5px 12px', fontWeight:900, fontSize:'0.78rem', cursor:'pointer', fontFamily:'var(--font-ui)',
            textTransform:'uppercase' }} aria-label="Back">â† Back</button>
          <span style={{ fontWeight:900, fontSize:'0.98rem', textTransform:'uppercase', letterSpacing:'0.05em' }}>ðŸŽ¯ Task Hub</span>
          {total > 0 && (
            <span style={{ fontSize:'0.7rem', fontFamily:'var(--font-mono)', background:'#000', color:'#f1be3e', padding:'2px 7px', fontWeight:700 }}>
              {total} tasks
            </span>
          )}
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          {overdueCount > 0 && (
            <span style={{ background:'#cc0000', color:'#fff', padding:'3px 9px', border:'2px solid #000', fontWeight:800, fontSize:'0.72rem' }}>
              âš ï¸ {overdueCount} Overdue
            </span>
          )}
          {completedToday > 0 && (
            <span style={{ background:'#00aa44', color:'#fff', padding:'3px 9px', border:'2px solid #000', fontWeight:800, fontSize:'0.72rem' }}>
              âœ… {completedToday} Done
            </span>
          )}
          <button onClick={handleEnableReminders}
            style={{ background: notifPerm === 'granted' ? '#00aa44' : '#fff',
              color: notifPerm === 'granted' ? '#fff' : '#000',
              border:'2px solid #000', padding:'5px 11px', fontWeight:700, fontSize:'0.75rem',
              cursor:'pointer', fontFamily:'var(--font-ui)', textTransform:'uppercase' }}>
            {notifPerm === 'granted' ? 'ðŸ”” On' : 'Enable Reminders'}
          </button>
          <button onClick={() => setShowForm(true)}
            style={{ background:'#000', color:'#f1be3e', border:'2px solid #000', padding:'6px 16px',
              fontWeight:900, fontSize:'0.85rem', cursor:'pointer', fontFamily:'var(--font-ui)',
              textTransform:'uppercase', boxShadow:'3px 3px 0 rgba(0,0,0,0.25)' }}>
            + New Task
          </button>
        </div>
      </nav>

      {/* â”€â”€ Split-screen body â”€â”€ */}
      <div style={{ flex:1, display:'flex', overflow:'hidden', minHeight:0 }}>

        {/* â•â•â•â• LEFT PANEL â€” Task list (60%) â•â•â•â• */}
        <div style={{ flex:'0 0 60%', display:'flex', flexDirection:'column', borderRight:'3px solid #000', overflow:'hidden' }}>

          {/* Filter bar â€” sticky inside left panel */}
          <div style={{ flexShrink:0, background:'#fff', borderBottom:'2px solid #000', padding:'12px 16px', display:'flex', flexDirection:'column', gap:10 }}>

            {/* Time filter tabs */}
            <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
              {TIME_FILTERS.map(tf => (
                <button key={tf.key} onClick={() => setTimeFilter(tf.key)}
                  style={{ padding:'5px 12px', border:'2px solid #000', fontWeight:700, fontSize:'0.72rem',
                    cursor:'pointer', fontFamily:'var(--font-ui)', textTransform:'uppercase',
                    background: timeFilter === tf.key ? '#000' : '#f4f1ea',
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              <input ref={searchRef} type="search" placeholder="ðŸ” Search tasksâ€¦" value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ flex:'1 1 160px', padding:'7px 10px', border:'2px solid #000', fontFamily:'var(--font-ui)',
                  fontSize:'0.82rem', background:'#f4f1ea', outline:'none', boxSizing:'border-box', minWidth:0 }} />
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)}
                style={{ padding:'7px 8px', border:'2px solid #000', fontFamily:'var(--font-ui)', fontSize:'0.78rem', background:'#f4f1ea', cursor:'pointer' }}>
              ...
              {courses.length > 0 && (
                <select value={filterCourse} onChange={e => setFilterCourse(e.target.value)}
                  ...
                  {courses.filter(c => !['data-analyst','data-analyst-en'].includes(c.id)).map(c =>
                    <option key={c.id} value={c.id}>{c.mascot || 'ðŸ“˜'} {c.title}</option>
                  )}
                </select>
              )}
              <label style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.75rem', fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'14px 16px' }}>
            ...
            {timeFilter === 'today' && suggestedTasks.length > 0 && !searchQuery && (
              <div style={{ background:'#fff3e0', border:'2px solid #e67e22', padding:'10px 14px', marginBottom:14 }}>
                <div style={{ fontWeight:800, fontSize:'0.75rem', textTransform:'uppercase', marginBottom:7, color:'#e67e22' }}>
                  âš¡ Suggested Next
                </div>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {suggestedTasks.map(t => (
                    <div key={t.id} style={{ display:'flex', alignItems:'center', gap:8, fontSize:'0.82rem', cursor:'pointer' }}
                      ...
                      <span style={{ flexShrink:0, fontSize:'0.65rem', background: getPriorityBg(t.priority),
                        padding:'1px 5px', border:`1px solid ${getPriorityColor(t.priority)}`,
                        color: getPriorityColor(t.priority), fontWeight:700 }}>
                        {t.priority.toUpperCase()}
                      </span>
                      <span style={{ fontWeight:600 }}>{t.title}</span>
                      {t.due_date && <span style={{ fontSize:'0.68rem', color:'#888', fontFamily:'var(--font-mono)' }}>{formatDueDate(t.due_date)?.label}</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Loading */}
            {loading && page === 1 && (
              <div style={{ textAlign:'center', padding:'40px 0', color:'#888', fontFamily:'var(--font-mono)', fontSize:'0.82rem' }}
                role="status">Loading tasksâ€¦</div>
            )}

            {/* Empty state */}
            {!loading && tasks.length === 0 && (
              <div style={{ textAlign:'center', padding:'50px 20px', border:'3px dashed #ccc' }}>
                <div style={{ fontSize:'2.8rem', marginBottom:10 }}>ðŸ“‹</div>
                <p style={{ fontWeight:700, fontSize:'0.95rem', margin:'0 0 5px' }}>
                  {searchQuery ? `No tasks matching "${searchQuery}"` : 'No tasks here yet'}
                </p>
                <p style={{ color:'#777', fontSize:'0.82rem', margin:'0 0 16px' }}>
                  {timeFilter === 'today' ? 'Nothing due today â€” try another filter.' : 'Add your first task above.'}
                </p>
                <button onClick={() => setShowForm(true)}
                  style={{ padding:'9px 22px', background:'#000', color:'#f1be3e', border:'2px solid #000',
                    fontWeight:900, fontSize:'0.85rem', cursor:'pointer', textTransform:'uppercase',
                    fontFamily:'var(--font-ui)', boxShadow:'4px 4px 0 #f1be3e' }}>
                  + Add Task
                </button>
              </div>
            )}
            ...
            {hasMore && (
              <div ref={loadMoreRef} style={{ height:36, display:'flex', alignItems:'center', justifyContent:'center' }}>
                {loading && <span style={{ color:'#888', fontFamily:'var(--font-mono)', fontSize:'0.78rem' }}>Loading moreâ€¦</span>}
              </div>
            )}
            <div style={{ height:24 }} />
          </div>
        ...
        {/* â•â•â•â• RIGHT PANEL â€” Analytics + Reminder Center (40%) â•â•â•â• */}
        <div style={{ flex:'0 0 40%', display:'flex', flexDirection:'column', overflow:'hidden', background:'#faf8f4' }}>

          {/* Right panel tab switcher */}
          <div style={{ flexShrink:0, display:'flex', borderBottom:'3px solid #000', background:'#fff' }}>
            {([
              { key:'analytics', label:'ðŸ“Š Analytics' },
              { key:'reminders', label:'ðŸ”” Reminder Center' },
            ] as const).map(tab => (
              <button key={tab.key} onClick={() => setRightTab(tab.key)}
                style={{ flex:1, padding:'12px 8px', border:'none',
                  borderRight: tab.key === 'analytics' ? '2px solid #000' : 'none',
                  fontWeight:900, fontSize:'0.8rem', cursor:'pointer',
                  fontFamily:'var(--font-ui)', textTransform:'uppercase', letterSpacing:'0.04em',
                  background: rightTab === tab.key ? '#000' : '#fff',
                  color: rightTab === tab.key ? '#f1be3e' : '#555',
          ...
          {/* Right panel content â€” scrollable */}
