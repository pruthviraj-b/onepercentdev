...
const VS = {
  ...
  text:      '#D4D4D4',
  textDim:   '#9CA3AF',
  comment:   '#6A9955', // success green
  keyword:   '#C586C0', // primary accent
  func:      '#DCDCAA', // secondary accent / badges
  string:    '#CE9178', // warnings / warm accent
  number:    '#B5CEA8', // stats / soft green
  danger:    '#F44747', // error red
};
    ...
    return (
      <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px',
        ...
        <a href={task.external_url} target="_blank" rel="noopener noreferrer"
          style={{ marginLeft:'auto', fontSize:'0.72rem', fontWeight:700, color: VS.bg, textDecoration:'none',
              ...
              {task.internal_link_id && onNavigate !== undefined && (
                <button onClick={() => onNavigate?.(task.internal_link_target!, task.internal_link_id!)}
                  style={{ marginLeft:8, padding:'2px 8px', fontSize:'0.72rem', border:`1px solid ${VS.keyword}`, borderRadius: 5,
                    background: VS.keyword, color: VS.bg, cursor:'pointer', fontWeight:700 }}>
                  Open →
                </button>
              )}
            </div>
...
function AnalyticsPanel({ analytics }: { analytics: TaskAnalytics | null }) {
  ...
  return (
    <div style={{ background: VS.bgRaised, border:`1px solid ${VS.border}`, borderRadius: 10, marginBottom:20, overflow:'hidden' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))', gap:0 }}>
        {stats.map((s, i) => (
          <div key={s.label} style={{ padding:'14px 16px', borderRight: i < stats.length-1 ? `1px solid ${VS.border}` : 'none' }}>
        ...
        <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {prefs.email_address && (
            <button type="button" onClick={onTestEmail} disabled={testingEmail} style={{
              ...
              fontFamily: FONT_UI, textTransform: 'uppercase',
              background: VS.keyword, color: VS.bg, width: '100%',
            }}>
      ...
      <nav className="taskhub-nav" style={{ flexShrink:0, background: VS.bgAlt, borderBottom:`1px solid ${VS.border}`,
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => setShowForm(true)}
            style={{ background: VS.keyword, color: VS.bg, border:'none', borderRadius: 6, padding:'7px 16px',
        ...
        <div className="taskhub-list-panel" style={{ flex:'0 0 60%', display:'flex', flexDirection:'column', borderRight:`1px solid ${VS.border}`, overflow:'hidden' }}>
          ...
          <div className="taskhub-task-scroll" style={{ flex:1, overflowY:'auto', padding:'14px 16px' }}>
            ...
            {timeFilter === 'today' && suggestedTasks.length > 0 && !searchQuery && (
              <div style={{ background:'rgba(206,145,120,0.1)', border:`1px solid ${VS.string}`, borderRadius: 8, padding:'10px 14px', marginBottom:14 }}>
            ...
            {!loading && tasks.length === 0 && (
              <div style={{ textAlign:'center', padding:'50px 20px', border:`2px dashed ${VS.border}`, borderRadius: 10 }}>
                ...
                <button onClick={() => setShowForm(true)}
                  style={{ padding:'9px 22px', background: VS.keyword, color: VS.bg, border:'none', borderRadius: 6,
