  ...
  if (!activeCourseId) {
    ...
    return (
      <div className="course-catalog-shell" style={{ minHeight: '100vh', background: C.bg, fontFamily: F.body, display: 'flex', color: C.text }}>
        ...
        {/* Sidebar */}
        <aside style={{ width: '224px', flexShrink: 0, background: C.surface, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <header style={{ height: '58px', flexShrink: 0, borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
                    ...
                    return (
                      <div key={mod.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                        <div className="course-module-header" onClick={() => toggleModule(mod.id)} style={{ padding: '13px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <span style={{ fontFamily: F.mono, fontSize: '0.68rem', color: C.textDim }}>{modDone}/{mod.notes.length}</span>
                            ...
                            {mod.notes.map(note => {
                              ...
                              return (
                                <div key={note.part}
                                  ...
                                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 18px 10px 46px', cursor: 'pointer' }}
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
                                    <div style={{
                                      ...
                                      background: isDone ? C.green : 'transparent', border: `1.5px solid ${isDone ? C.green : C.border}`,
                                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                                      fontFamily: F.mono, fontSize: '0.58rem', fontWeight: 700, color: isDone ? C.onAccent : C.textFaint,
                                    }}>{isDone ? '✓' : note.part}</div>
                                    <div style={{ minWidth: 0 }}>
                                      <div style={{ fontFamily: F.body, fontWeight: 600, fontSize: '0.8rem', color: isDone ? C.textFaint : C.text, textDecoration: isDone ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
