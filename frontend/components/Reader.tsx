...
const LearningWorkspace = dynamic(() => import('./LearningWorkspace').then(m => m.LearningWorkspace), { ssr: false });
...
// Newspaper typography & palette (Reader-specific)
const F = {
  masthead: "'Tapestry', cursive",
      ...
      <div className="reader-stage">
        ...
        {!loading && activeTab === 'notes' && mode === 'watch' && (
          <div className="reader-tool-page watch-page">
            {videoIds.length > 0 ? (
              /* Full-width LearningWorkspace handles its own video + side panel layout */
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
                <div style={{ padding: '10px 20px 6px', flexShrink: 0 }}>
                  <span className="note-part-badge">Part {noteData?.part ?? '-'}</span>
                  <span style={{ marginLeft: 10, fontWeight: 700, fontSize: '0.95rem' }}>{cleanTitle}</span>
                </div>
                <LearningWorkspace
                  videoId={videoIds[0]}
                  courseId={courseId}
                  partId={noteData?.part ?? 0}
                  courseTitle={noteData?.module}
                  lessonTitle={noteData?.title}
                  onAutoComplete={() => { if (!isCompleted) onToggleComplete(); }}
                />
              </div>
            ) : (
              /* No video — keep original layout */
              <>
  ...
  return (
    <div className="code-block-wrap">
      <div className="code-block-header">
        <span className="code-lang">{lang ? lang.toUpperCase() : 'CODE'}</span>
        <div className="code-block-actions">
          {onRun && (
            <button className="code-action-btn run-btn" onClick={onRun} title="Send to playground">
              Run
            </button>
          )}
          ...
          <button
            ...
            onClick={() => {
              ...
              const ext = ['python', 'py'].includes(l) ? 'py' : ['sql'].includes(l) ? 'sql' : ['bash', 'sh'].includes(l) ? 'sh' : 'txt';
