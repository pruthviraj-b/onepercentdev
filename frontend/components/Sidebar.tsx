...
import { C, F, R, S, T, L, FS, FONT_IMPORT } from '@/lib/theme';
...
export function Sidebar({
  ...
  const filtered = useMemo(() => {
    ...
    return modules.flatMap(m => m.notes.filter(n => n.title.toLowerCase().includes(q)));
  }, [modules, searchQuery]);

  const bookmarkedNotes = useMemo(() =>
    searchQuery.trim() ? [] : modules.flatMap(m => m.notes).filter(n => bookmarkedParts.includes(n.part)),
  ...
  const css = `
    ...
    .sb-brand-tag{
      ...
      border-radius:${R.sm};border:1.5px solid #000;box-shadow:1.5px 1.5px 0px #000;
    ...
    .sb-search-clear{
      ...
      background:${C.pink};border:1px solid #000;color:#fff;
    ...
    .sb-progress-pct{
      ...
      padding:3px 8px;border-radius:${R.sm};border:1.5px solid #000;
    ...
    .sb-current-actions{display:grid;grid-template-columns:1fr 1.35fr 1fr;gap:6px;margin-top:10px}
    .sb-current-action{min-width:0;padding:7px 5px;border:1px solid ${C.border};border-radius:${R.sm};background:${C.surfaceHi};color:${C.textDim};font-family:${F.mono};font-size:0.62rem;font-weight:800;cursor:pointer;white-space:nowrap}
    ...
    .sb-module-count.done{
      ...
      border-color:#000;font-weight:800;
      box-shadow:1.5px 1.5px 0px #000;
    }
    .sb-chevron{
    ...
    .sb-part.active .sb-part-icon{
      background:${C.accent};color:${C.onAccent};
      border-color:#000;box-shadow:1px 1px 0px #000;
    }
    ...
    .sb-part.active .sb-part-name{color:${C.accent}}
    ...
    .sb-active-badge{
      font-family:${F.mono};font-size:0.6rem;font-weight:800;
      background:${C.accent};color:${C.onAccent};
      padding:1px 5px;border-radius:${R.sm};border:1px solid #000;
    }
        ...
        {filtered ? (
          ...
          <>
            ...
            {modules.filter(m => m.notes.length > 0).map(mod => {
              ...
              const isComplete = doneCount === mod.notes.length;
              return (
                <div key={mod.id} className="sb-module">
                  ...
                  >
                    ...
                    <div className="sb-module-right">
                      <span className={`sb-module-count${isComplete ? ' done' : ''}`}>{doneCount}/{mod.notes.length}</span>
...
/* ─── Part Item ─────────────────────────────────────────────────────────────── */
function PartItem({
  note, active, done, bookmarked, query, onSelect,
}: {
  note: PartMeta; active: boolean; done: boolean; bookmarked: boolean; query?: string; onSelect: (p: number) => void;
}) {
  const shortTitle = note.title.replace(/^Part\s+\d+[\s—\-]+/i, '').trim();
  return (
    <li
      className={`sb-part${active ? ' active' : ''}${done ? ' done' : ''}`}
    ...
    >
      <div className="sb-part-left">
        <span className="sb-part-icon" aria-hidden="true">
          {done ? <IconCheck /> : note.part}
        </span>
