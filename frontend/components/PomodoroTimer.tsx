...
import { startPomodoroSession, completePomodoroSession } from '@/lib/learningPlayerApi';
...
export function PomodoroTimer({ courseId, partId, onSessionComplete }: PomodoroTimerProps) {
  ...
  return (
    <div style={{
        ...
        {(['work', 'break'] as Phase[]).map(p => (
          <button key={p} onClick={() => switchPhase(p)}
            style={{
              ...
              borderColor: phase === p ? '#000' : 'rgba(0,0,0,0.2)',
              background: phase === p ? '#000' : 'transparent',
              color: phase === p ? '#f1be3e' : '#555',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 12 }}>
        <svg width="100" height="100" viewBox="0 0 100 100">
          ...
          <circle cx="50" cy="50" r="40" fill="none"
            stroke={phase === 'work' ? '#f1be3e' : '#2ecc71'} strokeWidth="6"
            strokeDasharray={circumference}
            strokeDashoffset={circumference - (progress / 100) * circumference}
            strokeLinecap="round" transform="rotate(-90 50 50)"
            style={{ transition: 'stroke-dashoffset 1s linear' }}
          />
          <text x="50" y="46" textAnchor="middle" fill="#111"
            fontSize="18" fontWeight="900" fontFamily="var(--font-ui)">
            {mins}:{secs}
          </text>
          <text x="50" y="62" textAnchor="middle" fill="#888"
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
        {!running ? (
          <button onClick={handleStart}
            style={{ flex: 1, padding: '8px', background: '#000', color: '#f1be3e', border: '2px solid #000', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.75rem', cursor: 'pointer', textTransform: 'uppercase' }}>
            ▶ Start
          </button>
        ) : (
          <button onClick={handlePause}
            style={{ flex: 1, padding: '8px', background: 'transparent', color: '#000', border: '2px solid #000', fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: '0.75rem', cursor: 'pointer', textTransform: 'uppercase' }}>
            ⏸ Pause
          </button>
        )}
        <button onClick={handleReset}
          style={{ padding: '8px 12px', background: 'transparent', color: '#555', border: '2px solid rgba(0,0,0,0.2)', fontFamily: 'var(--font-ui)', fontWeight: 800, fontSize: '0.75rem', cursor: 'pointer' }}>
          ↺
        </button>
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <button onClick={() => set(Math.max(1, val - 5))}
                  style={{ width: 24, height: 24, background: 'transparent', border: '1px solid rgba(0,0,0,0.2)', color: '#111', cursor: 'pointer', fontSize: '0.9rem' }}>−</button>
                <span style={{ fontSize: '0.82rem', fontWeight: 800, minWidth: 24, textAlign: 'center', color: '#111' }}>{val}</span>
      ...
      {/* Stats */}
      {todaySessions > 0 && (
        <div style={{ marginTop: 8, borderTop: '1px solid rgba(0,0,0,0.1)', paddingTop: 8, fontSize: '0.7rem', color: '#888', textAlign: 'center' }}>
