export const colors = {
  brand: {
    50: '#fff7ed', 100: '#ffedd5', 200: '#fed7aa', 300: '#fdba74',
    400: '#fb923c', 500: '#f98012', 600: '#ea6a05', 700: '#c24f04',
    800: '#9a3d0b', 900: '#7c2d12',
  },
  neutral: {
    0: '#ffffff', 50: '#fafafa', 100: '#f4f4f5', 200: '#e4e4e7',
    300: '#d4d4d8', 400: '#a1a1aa', 500: '#71717a', 600: '#52525b',
    700: '#3f3f46', 800: '#27272a', 900: '#18181b', 950: '#09090b',
  },
  success: '#15803d', warning: '#b45309', error: '#b91c1c', info: '#0369a1',
  white: '#ffffff', black: '#000000',
  chart: ['#f98012', '#2563eb', '#16a34a', '#9333ea', '#db2777', '#0891b2'],
} as const;

export type ChartColor = (typeof colors.chart)[number];
