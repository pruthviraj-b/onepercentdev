...
# 3. LIBRARY — importing a third-party package installed via pip
import requests
...
if __name__ == "__main__":
    ...
    print("This project shows three levels of code reuse:\n")
    print("  1. MODULE  → my_module.py        (a single file you wrote)")
    print("  2. PACKAGE → my_package/          (a folder you organized)")
    print("  3. LIBRARY → requests             (code downloaded via pip)\n")

    demo_module()
    demo_package()
    demo_library()

    print_separator("What Did We Learn?")
    print("All three use the same 'import' keyword.")
    print("The only difference is WHERE the code lives:")
    print()
    print("  MODULE  → a .py file next to your script")
    print("  PACKAGE → a folder with __init__.py")
    print("  LIBRARY → installed from the internet (pip install)")
    print()
