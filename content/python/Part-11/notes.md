# Part 11 — String Formatting (f-strings, format(), %)

## Topic Overview

**String Formatting (f-strings, format(), %)** is a focused lesson in core Python. We will learn the operation, build a mental model, trace a small example, examine failure modes, and connect the technique to a real analytical decision.
...
- Define **String Formatting (f-strings, format(), %)** in precise technical language.
...
Analysts rarely fail because they cannot remember a method name. They fail when a technically valid operation answers the wrong question, silently drops records, changes the data grain, or hides an assumption. **String Formatting (f-strings, format(), %)** matters because it gives you a controlled way to solve a recurring problem in core Python.

## Definitions

**String Formatting (f-strings, format(), %)**: a Python/data-analysis technique used to express a specific transformation, inspection, calculation, or communication step. Its correctness depends on both its syntax and the meaning of its input.
...
```text
┌──────────────┐    ┌────────────────┐    ┌──────────────┐
│ input data   │ →  │ String Formatting (f-strings, format(), %)        │ →  │ result       │
...
3. Apply String Formatting (f-strings, format(), %)
...
**Why this output is correct:** Text is immutable, so the method returns a new string.
...
- Applying **String Formatting (f-strings, format(), %)** before confirming the input's type or grain.
...
1. What problem does **String Formatting (f-strings, format(), %)** solve?
...
1. Explain **String Formatting (f-strings, format(), %)** to a junior analyst using a small example.
...
**String Formatting (f-strings, format(), %)** is valuable when it is used deliberately: define the question, establish the input contract, apply the operation, inspect the output, validate the important invariants, and communicate the result with its assumptions. Syntax starts the work; reasoning and verification finish it.

## Revision Checklist

- [ ] I can define **String Formatting (f-strings, format(), %)** without copying a sentence.
