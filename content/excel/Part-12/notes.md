# Part 12 — Excel Sheet Protection, Hyperlinks, and Formatting

## Overview
This part covers sheet-level protection, creating navigable hyperlinks within and outside workbooks, and conditional formatting techniques that make data self-explanatory at a glance.

## Sheet Protection
Right-click sheet tab -> **Protect Sheet**
- Set a password (optional but recommended)
- Choose what users can still do: select locked/unlocked cells, insert rows, sort, use AutoFilter, etc.

## Unlock Specific Cells Before Protecting
By default, all cells are "locked" but locking only takes effect when the sheet is protected.

1. **Ctrl+A** to select all cells
2. **Ctrl+1** -> Protection tab -> uncheck **Locked**
3. Now select only the cells you want users to be unable to edit
4. **Ctrl+1** -> Protection tab -> check **Locked**
5. Protect the sheet: Review -> Protect Sheet

Result: only your specifically chosen cells are locked. All others remain editable.

## Protect Workbook Structure
**Review -> Protect Workbook**
- Prevents sheets from being added, deleted, renamed, moved, hidden, or unhidden
- Apply a password for stronger protection

## Hyperlinks — Insert with Ctrl+K
| Hyperlink Type | Use Case |
|---|---|
| Existing File or Web Page | Link to a URL or open another file |
| Place in This Document | Jump to a named cell or range in the same workbook |
| Create New Document | Create a new file and link to it |
| E-mail Address | Open email client with pre-filled To and Subject |

### Navigate Between Sheets
Ctrl+K -> Place in This Document -> type: `Summary!A1`
Creates a clickable link that jumps to cell A1 on the Summary sheet.
...
- Data Bars: length proportional to value
- Colour Scales: gradient from low (red) to high (green)
- Icon Sets: arrows, traffic lights, star ratings

### Custom Formula Rule
1. Home -> Conditional Formatting -> New Rule
2. Choose: Use a formula to determine which cells to format
3. Enter: `=A1>100` (applies the format when A1 exceeds 100)
4. Click Format -> choose colour/font/border -> OK

## Resources
- Watch: https://www.youtube.com/watch?v=6XWM6VBBt4g
