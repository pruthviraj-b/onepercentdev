# Part 10 — Master BI Skills Every Analyst Should Know

## Overview
Business Intelligence (BI) bridges raw Excel data and executive-level decision-making. This part introduces the BI mindset — the skills, tools, and thinking patterns that separate analysts who merely manage spreadsheets from those who drive decisions.

## What You Will Learn
- What Business Intelligence means in everyday practice
- The Excel BI stack from raw data to dashboard
- KPIs vs Metrics — the critical distinction
- How to turn raw data into actionable insights
- The complete data analyst workflow

## The Excel BI Stack
```
Raw Data (CSV, Database, API, Manual)
          |
    Power Query          <- Clean, shape, and transform data
          |
    Power Pivot          <- Build data model and relationships
          |
    DAX Measures         <- Calculate KPIs, running totals, YoY growth
          |
 PivotTable / PivotChart <- Summarise and visualise
          |
      Dashboard          <- Interactive, sliceable, shareable report
```

## KPIs vs Metrics — Know the Difference
| Term | Definition | Example |
|---|---|---|
| Metric | Any measurable value | Total Sales = Rs 4,50,000 |
| KPI | Metric measured against a target | Sales vs Target = 92% |

A metric becomes a KPI only when it is compared to a benchmark or goal.

## Common Business KPIs by Department
| Department | Key KPIs |
|---|---|
| Sales | Revenue vs Target, Win Rate, Average Deal Size |
| Marketing | CAC (Customer Acquisition Cost), Campaign ROI |
| Operations | SLA Compliance %, On-Time Delivery Rate |
| Finance | Gross Margin %, Burn Rate, Cash Runway |
| HR | Attrition Rate, Time to Hire, Training Hours |

## The Analyst Workflow (6 Steps)
1. **Ask** — Define the business question clearly
2. **Collect** — Import data from the source (file, DB, API)
3. **Clean** — Remove duplicates, fix formats, handle nulls and errors
4. **Analyse** — Apply formulas, PivotTables, statistical functions
5. **Visualise** — Build charts, dashboards, and conditional formatting
6. **Present** — Deliver clear, concise, actionable output to stakeholders

## Resources
- Watch: https://www.youtube.com/watch?v=Q_aLRZ_dryw
