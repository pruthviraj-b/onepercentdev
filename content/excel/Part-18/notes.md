# Part 18 — How to Create PivotCharts in Excel

## Overview
A PivotChart is a chart that is directly linked to a PivotTable. When you filter the PivotTable, the chart updates instantly. This is the foundation of building truly interactive Excel dashboards.

## PivotChart vs Regular Chart
| Feature | Regular Chart | PivotChart |
|---|---|---|
| Data source | Fixed cell range | PivotTable |
| Updates on data change | Only if source is a Table | Yes, with Refresh |
| Built-in filter buttons | No | Yes (hide for presentations) |
| Responds to Slicers | No | Yes |
| Multiple series easy | Yes | Yes |

## Create a PivotChart — Method 1: From an Existing PivotTable
1. Click anywhere inside your PivotTable
2. **PivotTable Analyze -> PivotChart**
3. Choose chart type and subtype -> OK

## Create a PivotChart — Method 2: From Scratch
1. Click any cell in your source data
2. **Insert -> PivotChart -> OK**
3. Excel creates **both** a PivotTable AND a PivotChart together

## Working with Field Filter Buttons
Filter buttons appear on the chart automatically.
- Click them to filter directly on the chart
- For presentations: right-click chart -> **Hide All Field Buttons on Chart**
- Use Slicers instead — cleaner and more interactive

## Formatting PivotCharts
Same tools as regular charts:
- **Chart Design** tab: layouts, colour styles, switch rows/columns
- **Format** tab: shape fill, outline, effects for individual elements
- Click chart title, axis, or legend to format each element individually

## Best PivotChart Type by Analysis
| Analysis Goal | PivotTable Setup | Recommended Chart |
|---|---|---|
| Compare regions | Rows: Region, Values: Revenue | Clustered Column |
| Show monthly trend | Rows: Month, Values: Sales | Line |
| Show category share | Values: Revenue (% of Grand Total) | Pie or Doughnut |
| Two metrics | Values: Revenue + Margin % | Combo (Column + Line) |
| Hierarchical breakdown | Rows: Region > City | Treemap |

## Disconnect a PivotChart
If you want a static chart based on the current pivot state:
Copy the chart -> Paste Special -> Picture (PNG)
Or: copy the PivotTable values as plain numbers, then build a regular chart.

## Resources
- Watch: https://www.youtube.com/watch?v=LaUeazdlHA0
